...
def load_particle_measurements(data_dir="./data/standardized_withNA/", 
    ...
    for gpkg_file in os.listdir(data_dir):
        if gpkg_file.endswith('.gpkg'):
            try:
                ...
                particle_cols = [col for col in df.columns 
...
def load_buildings(buildings_path="./data/osm/wue_buildings_and_landuse.gpkg"):
    ...
    buildings['height_calc'] = buildings['height'].fillna(
        pd.to_numeric(buildings['building:levels'], errors='coerce').fillna(3.) * 3.5  # 3.5m per level default
    )
...
def extract_aggregated_particle_size_bins(gdf):
    ...
    result['ultrafine_ufp'] = (gdf[[c for c in particle_cols if '0.3um' in c or '0.5um' in c or '1.0um' in c]]
                                .mean(axis=1, skipna=True))
    result['pm25_equiv'] = (gdf[[c for c in particle_cols if '2.5um' in c or '5.0um' in c or '10.0um' in c]]
                             .mean(axis=1, skipna=True))
    
    return pd.concat([gdf[['geometry']], result], axis=1)

def map_measurements_to_buildings(measurements_gdf, buildings_gdf, max_distance=100):
    """
    ...
    Args:
        ...
        max_distance: Maximum distance in meters to search for buildings
    ...
    distances, indices = building_tree.query(measurement_coords, k=5)
    
    # Initialize result columns
    measurements_gdf['nearest_building_height'] = np.nan
    measurements_gdf['nearest_building_distance'] = np.nan
    measurements_gdf['mean_5nn_building_height'] = np.nan
    measurements_gdf['max_5nn_building_height'] = np.nan
    measurements_gdf['street_canyon_index'] = np.nan
    
    for i, (dist, idx_list) in enumerate(zip(distances, indices)):
        valid_buildings = buildings_gdf.iloc[idx_list[dist <= max_distance/111000]]  # convert m to degrees
        ...
        if len(valid_buildings) > 0:
            ...
            measurements_gdf.loc[i, 'nearest_building_distance'] = dist[0] * 111000  # convert to meters
...
def calculate_urban_morphology_indices(measurements_gdf, buildings_gdf, buffer_radius=100):
    """
    ...
    for idx, point in measurements_gdf.iterrows():
        ...
        if len(buildings_in_buffer) > 0:
            ...
            measurements_gdf.loc[idx, 'sky_view_factor'] = 1 - (building_area / buffer_area)
...
def perform_correlation_analysis(measurements_gdf, output_dir="./results/"):
    ...
    particle_cols = [col for col in measurements_gdf.columns if 'particles_' in col or 'ufp' in col or 'pm25' in col]
    building_cols = ['nearest_building_height', 'mean_5nn_building_height', 
                     'max_5nn_building_height', 'building_density', 'sky_view_factor']
    ...
    for pollutant in particle_cols:
        for morph in building_cols:
            if pollutant in correlation_matrix.index and morph in correlation_matrix.columns:
                ...
                print(f"{pollutant:20s} vs {morph:30s}: r={r:7.4f}, p={p_value:.4e} {significance}")
    
    # Create correlation heatmap
    fig, ax = plt.subplots(figsize=(12, 8))
    sns.heatmap(correlation_matrix, annot=True, fmt='.2f', cmap='RdBu_r', 
                center=0, ax=ax, cbar_kws={'label': 'Pearson Correlation'})
...
def create_pollution_heatmap(measurements_gdf, buildings_gdf, 
                            pollutant_col='ultrafine_ufp', 
                            grid_resolution=100, output_path='pollution_heatmap.html'):
    """
    ...
    # Add heatmap layer
    heat_data = [[row.geometry.y, row.geometry.x, row[pollutant_col]] 
    ...
    for idx, row in valid_data.iterrows():
        ...
        folium.CircleMarker(
            ...
            radius=4,
            popup=
f"""Particles ufp: {row[pollutant_col]:.1f}
Particles : {row[""]:}
...
def create_building_height_pollution_plot(measurements_gdf, output_path='building_height_vs_pollution.png'):
    """
    ...
    for idx, col in enumerate(particle_cols):
        ...
        if len(valid) > 10:
            # Scatter plot
            ax.scatter(valid['nearest_building_height'], valid[col], 
                      alpha=0.5, s=30)
            
            # Regression line
            z = np.polyfit(valid['nearest_building_height'], valid[col], 1)
            p = np.poly1d(z)
            x_line = np.linspace(valid['nearest_building_height'].min(), 
                                valid['nearest_building_height'].max(), 100)
            ax.plot(x_line, p(x_line), "r--", linewidth=2, label='Linear fit')
            
            # Calculate R²
            r_squared = np.corrcoef(valid['nearest_building_height'], valid[col])[0, 1]**2
            
            ax.set_xlabel('Building Height (m)')
            ax.set_ylabel(col.replace('_', ' ').title())
            ax.set_title(f'{col.replace("_", " ").title()} vs Building Height (R²={r_squared:.3f})')
...
def create_density_heatmap_gridded(measurements_gdf, particle_col='ultrafine_ufp', 
                                   grid_size=100, output_path='density_heatmap_grid.png'):
    """
    ...
    scatter = ax.scatter(valid_data.geometry.x, valid_data.geometry.y, 
                        c=valid_data[particle_col], cmap='RdYlBu_r', 
...
def analyze_street_canyons(measurements_gdf, buildings_gdf, output_dir="./results/"):
    ...
    print("="*70)
    print(f"High-risk locations (High height + High pollution): {len(canyon_measurements)}")
    print(f"Mean building height in canyons: {canyon_measurements['nearest_building_height'].mean():.1f}m")
    print(f"Mean ultrafine particle concentration: {canyon_measurements['ultrafine_ufp'].mean():.1f} #/0.1L")
    
    # Visualize
    fig, ax = plt.subplots(figsize=(12, 8))
    
    scatter = ax.scatter(measurements_gdf['nearest_building_height'], 
                        measurements_gdf['ultrafine_ufp'],
                        c='blue', alpha=0.3, s=30, label='All measurements')
    ax.scatter(canyon_measurements['nearest_building_height'],
              canyon_measurements['ultrafine_ufp'],
              c='red', s=50, label='Street canyon hotspots', edgecolors='darkred', linewidth=1)
    
    ax.axvline(q75, color='gray', linestyle='--', label=f'75th percentile height ({q75:.1f}m)')
    ax.axhline(q75_ufp, color='orange', linestyle='--', label=f'75th percentile UFP ({q75_ufp:.1f})')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'street_canyon_analysis.png'), dpi=300, bbox_inches='tight')
    plt.close()
...
def generate_analysis_report(measurements_gdf, buildings_gdf, output_dir="./results/"):
    ...
    measurements_gdf = calculate_urban_morphology_indices(measurements_gdf, buildings_gdf)
    ...
    create_building_height_pollution_plot(measurements_gdf, 
                                         os.path.join(output_dir, 'building_height_vs_pollution.png'))
    create_density_heatmap_gridded(measurements_gdf, 'ultrafine_ufp',output_path=
                                  os.path.join(output_dir, 'ufp_heatmap.png'))
    create_density_heatmap_gridded(measurements_gdf, 'pm25_equiv', output_path=
                                  os.path.join(output_dir, 'pm25_heatmap.png'))
    create_pollution_heatmap(measurements_gdf, buildings_gdf,output_path=
                            os.path.join(output_dir, 'interactive_pollution_map.html'))
    ...
    print(f"Mean nearest building height: {measurements_gdf['nearest_building_height'].mean():.1f}m")
    print(f"Mean ultrafine particle concentration: {measurements_gdf['ultrafine_ufp'].mean():.1f} #/0.1L")
    print(f"Mean PM2.5 equivalent: {measurements_gdf['pm25_equiv'].mean():.1f} #/0.1L")
    print(f"Building density (buildings/ha): {measurements_gdf['building_density'].mean():.2f}")
    
    # Save processed data
    measurements_gdf.to_file(os.path.join(output_dir, 'measurements_with_building_features.gpkg'))
